...
function facebook(url) {
    return new Promise(async (resolve, reject) => {
        ...
        axios.post(host + '/system/action.php', qs.stringify(form.data), { headers: is.headers })
            .then(({ data }) => {
                if (data == 'error') return resolve({ creator: '𝐒𝐨𝐧𝐲 𝐂𝐨𝐯𝐞𝐫𝐲', status: false, message: 'cant be downloaded' })
                resolve({ creator: '𝐒𝐨𝐧𝐲 𝐂𝐨𝐯𝐞𝐫𝐲', status: true, thumb: data.thumbnail, data: data.links })
            })
    })
}
